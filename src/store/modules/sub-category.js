...
export default {
    ...
    actions: {
        async getCategoryList({ commit }) {
            const res = await axios.get('sub-category/list')

            commit('SET_CATEGORIES', res.data.sub_categories)
        },

        
        async createCategory({ commit }, formdata) {
            const res = await axios.post('sub-category/create', formdata)
            if (res.data.status) {
                commit('SET_CATEGORY', res.data.sub_category)
        ...
        async updateCategory({ commit }, formdata) {
            const res = await axios.put('sub-category/edit/'+formdata.sub_category_id, formdata)
            if (res.data.status) {
                commit('UPDATE_CATEGORY', res.data.sub_category)
                commit('SET_VALIDATION_ERRORS', null,  { root:true })
                commit('SET_ERROR_MESSAGE', null,  { root:true })
            }else {
                if (res.data.errors) {
        ...
        async deleteCategory({ commit }, category_id) {
            const res = await axios.delete('sub-category/delete/'+category_id)
            if (res.data.status) {
